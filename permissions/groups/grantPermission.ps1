################################################################
# HelloID-Conn-Prov-Target-Myneva-GrantPermission-Group
# PowerShell V2
################################################################
...
try {
    ...
    if ($null -ne $correlatedAccount) {
        ...
        $dryRunMessage = "Grant Myneva permission: [$($actionContext.References.Permission.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        switch ($action) {
            'GrantPermission' {
                Write-Information "Granting Myneva permission: [$($actionContext.References.Permission.DisplayName)] - [$($actionContext.References.Permission.Reference)]"
                $null = Set-RegasGroupAssignment -PersonId $actionContext.References.Account -UserGroupId $actionContext.References.Permission.Reference
    ...
    if ($null -ne $($errorObject.InnerException)) {
        $auditMessage = "Could not grant Myneva permission [$($actionContext.References.Permission.DisplayName)]. Error: $($errorObject.InnerException)"
    } else {
        $auditMessage = "Could not grant Myneva permission [$($actionContext.References.Permission.DisplayName)]. Error: $($errorObject.message)"
    }
    $outputContext.AuditLogs.Add([PSCustomObject]@{
